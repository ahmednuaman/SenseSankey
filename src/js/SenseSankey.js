...
import d3 from 'd3'
import md5 from 'md5'
import senseSankey from './sankeymore'

export const paint = function ($element, layout) {
  ...
  node.on('click', function (d, i) {
    ...
    _this.backendApi.selectValues(
            parseInt(d.name.split('~')[1].replace('end', qDim.length - 1)), [parseInt(d.name.split('~')[0].split('|')[1])],
            true
        )
  })
  ...
  link.on('click', function (d, i) {
    ...
    _this.backendApi.selectValues(
            parseInt(d.source.name.split('~')[1].replace('end', qDim.length - 1)), [parseInt(d.source.name.split('~')[0].split('|')[1])],
            true
        )
